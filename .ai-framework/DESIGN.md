...
- **Largura da leitura** (`.markdown-body--fluid`): o painel de configuracoes define a largura de leitura em porcentagem (20-100%, passos de 5%, persistido, padrao 40%), aplicada via `--reading-width` no elemento; o preview mantem piso legivel de `--reading-width-min`, limitado ao espaco disponivel em janelas estreitas. Botao iconico disponivel somente no preview alterna entre a largura configurada e a largura total do painel. Estado ativo usa `--accent`; a alternancia vale somente na sessao atual e cada inicializacao retorna a largura configurada.

...
The standard convention uses **LaTeX** between dollar signs: `$...$` for **inline** formulae and `$$...$$` for **display** (block, centred) formulae.
...
> **In Moji:** formulae are rendered with **KaTeX** — `$…$` appears inline and `$$…$$` in a centred display block. Wide equations get horizontal scrolling, and an invalid formula becomes red error text without breaking the rest of the document.
